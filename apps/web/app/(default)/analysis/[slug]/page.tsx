...
export async function generateMetadata({ params }: AnalysisRouteProps) {
  ...
  if (slug === "policy-coverage") {
    return {
      ...
      alternates: getLocalizedAlternates(`/analysis/${slug}`, "en"),
      openGraph: {
                ...
                <tr key={profile.entitySlug}>
                  <td>
                    <div className="table-record-title">
                      <Link
                        href={localizeHref(
                          `/universities/${profile.entitySlug}`,
                          locale
                        )}
                      >
                  ...
                  <td>
                    <div className="analysis-link-stack">
                      <Link
                        href={localizeHref(
                          `/universities/${profile.entitySlug}#policy-profile`,
                          locale
                        )}
                      >
                        Profile
                      ...
                      {dimension.basis[0] ? (
                        <Link
                          href={localizeHref(
                            `/universities/${profile.entitySlug}#claim-${dimension.basis[0].claimId}`,
                            locale
                          )}
                        >
                          Claim evidence
...
async function PolicyCoveragePage({ locale }: { locale: SupportedLocale }) {
  ...
  const canonical = getAbsoluteSiteUrl(
    localizeHref("/analysis/policy-coverage", locale)
  );
