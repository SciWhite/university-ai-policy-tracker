...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: getLocalizedAlternates(canonicalPath, "en"),
    openGraph: {
      title,
      description,
      url: canonical,
...
export default async function UniversityAiPolicyDatabasePage({
  ...
  const stats = await getDatabaseStats();
  const title = buildTitle(stats.universityCount, locale);
  const canonical = getAbsoluteSiteUrl(canonicalPath);
  ...
  return (
    <main className="page-shell page-shell--wide">
      ...
      <section className="metrics-grid" aria-label="Public database coverage">
        <div>
          <span>{formatNumber(stats.universityCount, locale)}</span>
          <p>public university records</p>
        </div>
        <div>
          <span>{formatNumber(stats.claimCount, locale)}</span>
          <p>source-backed claims</p>
        </div>
        <div>
          <span>{formatNumber(stats.evidenceCount, locale)}</span>
          <p>claim evidence records</p>
        </div>
        <div>
          <span>{formatNumber(stats.sourceCount, locale)}</span>
...
function buildTitle(universityCount: number, locale = "en"): string {
  return `University AI Policy Database: Search ${formatNumber(universityCount, locale)} Source-Backed GenAI Policies`;
}
