  ...
  function openMetric(
    ...
    openDrawer({
      ...
      rows: [
        ...
        [t.previous, formatMetricValue(key, previousValue)],
        [locale === "zh" ? "变化" : "Change", formatChange(currentValue, previousValue, t.new)]
      ],
      title
    });
  }
  ...
  return (
    <main className="analytics-dashboard" lang={locale === "zh" ? "zh-CN" : "en"}>
      ...
      <section className="analytics-kpi-grid" aria-label="Dashboard metrics">
        <KpiCard current={current.summary.visitors} label={t.visitors} onClick={() => openMetric("visitors", t.visitors, current.summary.visitors, previous.summary.visitors)} previous={previous.summary.visitors} spark={current.summary.trend.map((row) => row.visitors)} />
        <KpiCard current={current.summary.sessions} label={t.sessions} onClick={() => openMetric("sessions", t.sessions, current.summary.sessions, previous.summary.sessions)} previous={previous.summary.sessions} spark={current.summary.trend.map((row) => row.sessions)} />
        <KpiCard current={current.summary.pageViews} label={t.pageViews} onClick={() => openMetric("pageViews", t.pageViews, current.summary.pageViews, previous.summary.pageViews)} previous={previous.summary.pageViews} spark={current.summary.trend.map((row) => row.pageViews)} />
        <KpiCard current={engagedRate} format="percent" label={t.engaged} onClick={() => openMetric("rate", t.engaged, engagedRate, previousEngagedRate)} previous={previousEngagedRate} spark={current.summary.trend.map((row) => safeRate(row.events - row.pageViews, row.sessions))} />
        <KpiCard current={gsc.totals.clicks} label={t.clicks} onClick={() => openMetric("clicks", t.clicks, gsc.totals.clicks, previousGsc.totals.clicks)} previous={previousGsc.totals.clicks} spark={gsc.dateRows.map((row) => row.clicks)} />
        <KpiCard current={gsc.totals.impressions} label={t.impressions} onClick={() => openMetric("impressions", t.impressions, gsc.totals.impressions, previousGsc.totals.impressions)} previous={previousGsc.totals.impressions} spark={gsc.dateRows.map((row) => row.impressions)} />
        <KpiCard current={gsc.totals.ctr} format="percent" label={t.ctr} onClick={() => openMetric("rate", t.ctr, gsc.totals.ctr, previousGsc.totals.ctr)} previous={previousGsc.totals.ctr} spark={gsc.dateRows.map((row) => row.ctr * 100)} />
        <KpiCard current={gsc.totals.position} format="decimal" label={t.avgPosition} onClick={() => openMetric("position", t.avgPosition, gsc.totals.position, previousGsc.totals.position)} previous={previousGsc.totals.position} reverseTone spark={gsc.dateRows.map((row) => row.position)} />
      </section>
        ...
        <DashboardPanel index="02" meta={t.onsite} title={t.onsiteTrend}>
          <DashboardLineChart
            ...
            series={[
              ...
              ...(query.compare ? [{ color: "var(--color-text-muted)", dashed: true, key: "previous", label: t.previous, values: previous.summary.trend.map((row) => row.visitors) }] : [])
            ]}
          />
        </DashboardPanel>
        <DashboardPanel
        ...
        >
          <DashboardLineChart
            ...
            series={[
              ...
              ...(query.compare ? [{ color: "var(--color-text-muted)", dashed: true, key: "previous", label: t.previous, values: previousGsc.dateRows.map((row) => metricValue(row, gscMetric)) }] : [])
            ]}
          />
        </DashboardPanel>
      ...
      <section className="analytics-detail-sections">
        ...
        <DetailSection index="09" title={t.sourcesGeo}>
        ...
        <DetailSection index="10" title={t.quality}>
...
function KpiCard({ current, format = "count", label, onClick, previous, reverseTone = false, spark }: { current: number; format?: "count" | "decimal" | "percent"; label: string; onClick: () => void; previous: number; reverseTone?: boolean; spark: number[] }) {
  const change = previous ? (current - previous) / previous : current ? null : 0;
  const positive = change !== null && (reverseTone ? change < 0 : change >= 0);
  return <button className="analytics-kpi-card" onClick={onClick} type="button"><span>{label}</span><strong>{format === "percent" ? formatPercent(current) : format === "decimal" ? formatDecimal(current) : formatCount(current)}</strong><em className={change === null ? "is-new" : positive ? "is-positive" : "is-negative"}>{change === null ? "NEW" : formatSignedPercent(change)}</em><DashboardSparkline values={spark} /></button>;
}
...
function AnalyticsDrawer({ drawer, locale, onClose, t }: { drawer: DrawerState; locale: DashboardLocale; onClose: () => void; t: typeof copy.en | typeof copy.zh }) {
  return <div className="analytics-drawer-layer"><button aria-label={t.close} className="analytics-drawer-backdrop" onClick={onClose} type="button" /><aside aria-modal="true" className="analytics-drawer" role="dialog"><header><div><span>{drawer.focus}</span><h2>{drawer.title}</h2></div><button onClick={onClose} type="button">{t.close}</button></header><div className="analytics-drawer-comparison"><article><span>{locale === "zh" ? "本期" : "Current"}</span><strong>{formatCount(drawer.current)}</strong></article><article><span>{t.previous}</span><strong>{formatCount(drawer.previous)}</strong></article><article><span>{locale === "zh" ? "变化" : "Change"}</span><strong>{formatChange(drawer.current, drawer.previous, t.new)}</strong></article></div><dl>{drawer.rows.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></aside></div>;
}
